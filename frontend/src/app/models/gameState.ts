...
export default class GameState {
    ...
    public generateBase64Dictionary(deckName: string, api:boolean=true, payload = null, playspaceComponent:PlayspaceComponent = null) {
        if (this.amHost) {
                ...
                if(api == true) {
                    ...
                    for(let i = 0; i < payload.ids.length; i ++) {
                        if ( arrayOfBase64 == undefined || !(payload.ids[i] in arrayOfBase64) ) {
                            arrayOfBase64[payload.ids[i]] = payload.cards[i].base64;
                        } else {
            ...
            console.log("generated Dictionary");
            console.log(this.base64Dictionary);
        }
    }

    
// Doesnt use Ids anymore
                ...
                for (let i: number = 0; i < this.connections.length; i++) {
                    if (playerData.peerID === this.connections[i].peer) {
                        if (((onlySendTo !== "" && onlySendTo === playerData.peerID) || onlySendTo === "") && (doNotSendTo === "" || (doNotSendTo !== playerData.peerID))) {
                            ...
                            this.connections[i].send(new GameObjectProperties(this.amHost, EActionTypes.replicateState, this.myPeerID, this.playerID, { 'state': sentGameState, 'base64Dictionary': this.base64Dictionary, 'texturepack': this.texturepack, 'undo': undo }));
                            break; 
                        }
                    }
                }
            ...
            receivedGameState.handMins.forEach((handMin: HandMin, handIndex: integer) => {
              ...
              handMin.cardMins.forEach((cardMin: CardMin) => {
                if(cardMin.base64 == false) {  
                    ...
                    let card: Card = new Card(cardMin.id, null, cardMin.x, cardMin.y, cardMin.flippedOver, true);
                    card.base64 = true;
                    card.base64Deck = cardMin.deckName
                    card.base64Id = cardMin.id;
                    HF.createCard(card, playspaceComponent, HF.EDestination.HAND, cardMin.depth, handIndex, true);
            ...
            case EActionTypes.sendTopCard:
                ...
                if (deck) {
                    ...
                    if (card.base64 == false) {
                        ...
                        HF.createCard(card, playspaceComponent, data.extras.destination, 0, null, true);
                    }
                }
                }
                break;
